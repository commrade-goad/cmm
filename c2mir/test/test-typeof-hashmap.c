...
int main() {
    ...
    /* --- update --- */
    double upd = 999.0;
    hm_set(&hm, keys[0], upd);
    double *v = hm_get(&hm, keys[0]);
    if (*v != 999.0) { printf("FAIL: update\n"); pass = 0; }
    else printf("\n  updated %d -> %g\n", keys[0], *v);

    /* --- missing key --- */
    int missing = 888;
    if (hm_has(&hm, missing)) { printf("FAIL: false positive\n"); pass = 0; }
    printf("  missing %d -> %s\n", missing, hm_get(&hm, missing) == NULL ? "NULL" : "FOUND");

    /* --- remove --- */
    hm_remove(&hm, keys[1]);
    if (hm_has(&hm, keys[1])) { printf("FAIL: remove didn't work\n"); pass = 0; }
    printf("  after remove %d: has=%s count=%d\n", keys[1],
           hm_has(&hm, keys[1]) ? "yes" : "no", hm.count);
