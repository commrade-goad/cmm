...
(defun cmm-types ()
  '("char" "int" "long" "short" "void" "bool" "float" "double" "signed" "unsigned"
    ...
    "uintptr_t" "size_t" "ptrdiff_t" "va_list" "__type_t"))
...
(defun cmm--syntax-propertize (start end)
  "Lexer for Rust-style raw strings: r\"...\", r#\"...\"#, etc."
  (remove-text-properties start end '(syntax-table nil))
  (goto-char start)
  (while (re-search-forward "\\(r\\(#*\\)\\(\"\\)\\)" end t)
    (let* ((hashes (match-string 2))
           (n (length hashes))
           (opener-start (match-beginning 3))
           (opener-end (match-end 3))
           (closing-regex (concat "\"\\(" (make-string n ?#) "\\)")))
      (when (re-search-forward closing-regex nil t)
        (let ((closer-start (match-beginning 0))
              (closer-end (match-end 0)))
          (put-text-property opener-start opener-end 'syntax-table (string-to-syntax "|"))
          (put-text-property closer-start (1+ closer-start) 'syntax-table (string-to-syntax "|"))
...
(defun cmm--previous-non-empty-line ()
  (save-excursion
    ...
    (if (bobp) nil
      (forward-line -1)
      (while (and (not (bobp)) (string-empty-p (string-trim-right (thing-at-point 'line t))))
        (forward-line -1))
...
(defun cmm--desired-indentation ()
  (let ((prev (cmm--previous-non-empty-line)))
    (if (not prev) (current-indentation)
      (let ((indent-len 4)
            (cur-line (string-trim-right (thing-at-point 'line t)))
            (prev-line (string-trim-right (car prev)))
            (prev-indent (cdr prev)))
        (cond
         ((string-match-p "^\\s-*switch\\s-*(.+)" prev-line) prev-indent)
         ((and (string-suffix-p "{" prev-line) (string-prefix-p "}" (string-trim-left cur-line))) prev-indent)
         ((string-suffix-p "{" prev-line) (+ prev-indent indent-len))
         ((string-prefix-p "}" (string-trim-left cur-line)) (max (- prev-indent indent-len) 0))
         ((string-suffix-p ":" prev-line) (if (string-suffix-p ":" cur-line) prev-indent (+ prev-indent indent-len)))
         ((string-suffix-p ":" cur-line) (max (- prev-indent indent-len) 0))
         (t prev-indent))))))

(defun cmm-indent-line ()
  (interactive)
  (when (not (bobp))
    (let* ((desired-indentation (cmm--desired-indentation))
...
(define-derived-mode cmm-mode prog-mode "cmm"
  "Major mode for editing cmm."
  :syntax-table cmm-mode-syntax-table
  (setq-local syntax-propertize-function #'cmm--syntax-propertize)
